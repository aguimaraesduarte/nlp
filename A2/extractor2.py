...
class extractor(object):
	...
	def __init__(self, wiki_url_, bands_list_, train_ner_, train_rel_, threshold_, ner_model_name_, rel_b2m_model_name_, rel_m2b_model_name_, mitie_path_):
		"""
		Constructor
		"""
		...
		self.REL_B2M_MODEL_NAME = rel_b2m_model_name_
		self.REL_M2B_MODEL_NAME = rel_m2b_model_name_
	...
	def findPositions(self, group_, tokens_, sample_, trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_, grp2_indices_, rel_):
		import sys
	...
	def iteratePhrases(self, phrases_, rel_, rel2_, trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_, grp2_, grp_token_):
		import sys
		# sys.path.append(self.MITIE_PATH + 'mitielib')
		...
		for phrase in phrases_:
			...
			if len(tokens) > 0:
				...
				self.findPositions(grp2_, tokens, sample, trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_, grp_indices, rel2_)


	def extractFromHtml(self, cpath_, grpname_, grp_, grp2_, trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_):
		import sys
		# sys.path.append(self.MITIE_PATH + 'mitielib')
		from mitie import tokenize
		"""
		...
		self.iteratePhrases(phrases, grp_, grp2_, trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_, grp, grp_token)
	...
	def modelLoading(self, trainer_ner_, fun_ner_, trainer_rel_b2m_, trainer_rel_m2b_, fun_rel_):
		"""
		...
		rel_model_b2m = self.trainLoadModel(self.TRAIN_REL, "REL", trainer_rel_b2m_, fun_rel_, self.REL_B2M_MODEL_NAME)
		rel_model_m2b = self.trainLoadModel(self.TRAIN_REL, "REL", trainer_rel_m2b_, fun_rel_, self.REL_M2B_MODEL_NAME)

		return ner_model, rel_model_b2m, rel_model_m2b


	def modelTraining(self, bands_list_, cpath_, trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_):
		"""
		...
		for band in bands_list_:
			...
			try:
				members = self.extractFromHtml(cpath_, band, "band", "person", trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_)
			except:
				pass
			...
			for member in members:
				...
				try:
					self.extractFromHtml(cpath_, member, "person", "band", trainer_ner_, cond_NER_, trainer_rel_b2m_, trainer_rel_m2b_, cond_REL_)
				except:
					pass
	...
	def training(self, cpath_):
		...
		if self.TRAIN_REL:
			...
			========================
			"""
			ner = named_entity_extractor(self.MITIE_PATH + 'MITIE-models/english/ner_model.dat')
		...
		if self.TRAIN_NER or self.TRAIN_REL:
			...
			========================
			"""
			self.modelTraining(self.BANDS, cpath_, trainer_ner, self.TRAIN_NER, trainer_rel_b2m, trainer_rel_m2b, self.TRAIN_REL)

		return trainer_ner, trainer_rel_b2m, trainer_rel_m2b


	def testing(self, cpath_, ner_model_, rel_model_b2m_, rel_model_m2b_):
		import sys
		"""
		...
		while user_input != "quit":
			...
			if user_input.startswith("person "):
				...
				self.tryExtract(member, "member", cpath_, ner_model_, rel_model_m2b_, self.SCORE_THRESHOLD)

			elif user_input.startswith("band "):
				band = user_input.split("band ")[1]
				self.tryExtract(band, "band", cpath_, ner_model_, rel_model_b2m_, self.SCORE_THRESHOLD)
	...
	def main(self):
		...
		# TRAINING
		trainer_ner, trainer_rel_b2m, trainer_rel_m2b = self.training(cache_path)

		# train/load models
		ner_model, rel_model_b2m, rel_model_m2b = self.modelLoading(trainer_ner, named_entity_extractor, trainer_rel_b2m, trainer_rel_m2b, binary_relation_detector)

		# TESTING
		self.testing(cache_path, ner_model, rel_model_b2m, rel_model_m2b)
